<script lang="ts">
  ...
  //
  // Rendered as an on-demand overlay drawer (fixed, right-side) that appears only
  // when something is being inspected. The cockpit keeps full width at all times.
...
{#if data}
  <!-- Floating overlay drawer pinned to the right, below the topbar (h-12 = 3rem) -->
  <aside
    class="fixed top-12 right-0 bottom-0 w-[320px] z-40 flex flex-col
           border-l border-[var(--color-border)] bg-[var(--color-panel)]"
    style="box-shadow: -4px 0 16px rgba(0,0,0,0.4);"
  >
    <header
